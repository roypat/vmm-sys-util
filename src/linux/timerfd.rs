...
impl TimerFd {
    ...
    pub fn new_with_flags(tfd_flags: TimerFdFlag) -> Result<TimerFd> {
        let mut flags = 0;
        if tfd_flags.contains(TimerFdFlag::CLOEXEC) {
            flags |= libc::TFD_CLOEXEC;
        }
        if tfd_flags.contains(TimerFdFlag::NONBLOCK) {
            flags |= libc::TFD_NONBLOCK;
        }
        // SAFETY: Safe because this doesn't modify any memory and we check the return value.
        let ret = unsafe { timerfd_create(CLOCK_MONOTONIC, flags) };
        if ret < 0 {
            return errno_result();
        }
